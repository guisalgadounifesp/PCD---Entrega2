...
int GetEstadoNovo(int **grid, int line, int column)
{
...
#pragma omp for
            for (i = 0; i < N; i++)
            {
                for (j = 0; j < N; j++)
                {
                    ProximoGrid[i][j] = GetEstadoNovo(GridAtual, i, j);
                }
            }
        }
    }
...
int GetSobreviventes(int **grid)
{
    int vivo = 0;
    int i, j;
    #pragma omp parallel for default(none) shared(grid) private(i, j) reduction(+:vivo)
